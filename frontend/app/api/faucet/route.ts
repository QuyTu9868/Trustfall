 ...
 */
/**
 * Enough for the most expensive thing listed, and not a fortune.
 *
 * It was ten thousand, which is a number nobody in a marketplace of hundred-USDC scooters
 * has any use for and which makes the money on screen read as play money. Five hundred
 * covers the dearest listing here, a hundred a day against a hundred deposit, for a couple
 * of days with room over.
 *
 * Note what lowering it does NOT do: it saves no gas. A mint costs the same to send whatever
 * number is in it, and a smaller grant means somebody comes back for a second one sooner, so
 * if anything this is very slightly more transactions. The reason to do it is that the demo
 * reads as a market rather than a sandbox.
 */
const TOP_UP = "500";

/** Below this, top somebody up. Above it, they can afford to rent something already. */
const ENOUGH = "100";
