...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(`/api/reviews?about=${address.toLowerCase()}`);
  ...
  const renting = rentals.filter((rental) => rental.renter.toLowerCase() === me);
        ...
        <div className="flex items-baseline gap-3">
          ...
          {average !== null && (
            <span className="text-sm text-ink-muted">
              <span className="tabular">{average.toFixed(1)}</span> from {reviews?.length}
            </span>
          )}
        </div>
